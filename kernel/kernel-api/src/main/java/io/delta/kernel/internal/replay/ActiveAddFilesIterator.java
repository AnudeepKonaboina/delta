...
public class ActiveAddFilesIterator implements CloseableIterator<FilteredColumnarBatch> {
  ...
  private void prepareNext() {
    ...
    // Keep consuming underlying batches until we find one with at least one active AddFile.
    // If a batch contains zero active AddFiles, skip creating a FilteredColumnarBatch for it.
    while (!next.isPresent() && iter.hasNext()) {
      ...
      int numActiveAddFilesInBatch = 0;
      ...
      // If there are no active AddFiles in this batch, skip creating a FilteredColumnarBatch.
      // This prevents Scan#getScanFiles (and PaginatedScan#getScanFiles) from returning batches
      // with zero selected rows.
      if (numActiveAddFilesInBatch == 0) {
        continue;
      }
