...
class DeltaVacuumSuite extends DeltaVacuumSuiteBase with DeltaSQLCommandTest {
  ...
  test("vacuum does not delete tombstoned files outside the table root by default") {
    withEnvironment { (tempDir, clock) =>
      val outsideDir = java.nio.file.Files.createTempDirectory("outside_data").toFile
      try {
        val table = DeltaTableV2(spark, tempDir, clock)
        val outsideFile = new File(outsideDir, "outsideFile.txt").getAbsolutePath
        gcTest(table, clock)(
          CreateFile(outsideFile, commitToActionLog = true),
          CheckFiles(Seq(outsideFile)),
          LogicallyDeleteFile(outsideFile),
          AdvanceClock(1),
          GC(dryRun = false, expectedDf = Seq(tempDir), retentionHours = Some(0)),
          CheckFiles(Seq(outsideFile), exist = true)
        )
      } finally {
        org.apache.spark.util.Utils.deleteRecursively(outsideDir)
      }
    }
  }

  test("vacuum can delete tombstoned files outside the table root when enabled with allowlist") {
    withEnvironment { (tempDir, clock) =>
      val outsideDir = java.nio.file.Files.createTempDirectory("outside_data").toFile
      try {
        val outsidePrefix = new Path(outsideDir.getAbsolutePath).toUri.toString
        val table = DeltaTableV2(spark, tempDir, clock)
        val outsideFile = new File(outsideDir, "outsideFile.txt").getAbsolutePath
        withSQLConf(
          DeltaSQLConf.DELTA_VACUUM_DELETE_OUTSIDE_TABLE_ENABLED.key -> "true",
          DeltaSQLConf.DELTA_VACUUM_DELETE_OUTSIDE_TABLE_ALLOWED_PREFIXES.key -> outsidePrefix
        ) {
          gcTest(table, clock)(
            CreateFile(outsideFile, commitToActionLog = true),
            CheckFiles(Seq(outsideFile)),
            ...
            AdvanceClock(1),
            GC(dryRun = false, expectedDf = Seq(tempDir), retentionHours = Some(0)),
            CheckFiles(Seq(outsideFile), exist = false)
          )
        }
      } finally {
        org.apache.spark.util.Utils.deleteRecursively(outsideDir)
  ...
  test("vacuum outside-table delete requires allowlist") {
    withEnvironment { (tempDir, clock) =>
      val outsideDir = java.nio.file.Files.createTempDirectory("outside_data").toFile
      try {
        val table = DeltaTableV2(spark, tempDir, clock)
        val outsideFile = new File(outsideDir, "outsideFile.txt").getAbsolutePath
        withSQLConf(DeltaSQLConf.DELTA_VACUUM_DELETE_OUTSIDE_TABLE_ENABLED.key -> "true") {
          gcTest(table, clock)(
            CreateFile(outsideFile, commitToActionLog = true),
            LogicallyDeleteFile(outsideFile),
            AdvanceClock(1),
